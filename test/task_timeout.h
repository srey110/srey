...
// 每轮依次执行八个子测试，任意失败则将 *ok 置 0，全部通过置 1：
...
//   5. UDP busy：两个协程 fork_wait 并发共享同一 skid，验证后发起的 coro_sendto 被拒绝而非崩溃
//   6. TCP：PACK_CUSTZ_FIXED / FLAG / VAR 各 ECHO_ROUNDS 轮，含 SSL 升级和协议切换
//   7. HTTP：GET 请求验证 200 响应；chunked POST 请求收发验证（三帧往返）
//   8. WS：纯 WebSocket 服务端文本帧、二进制帧回显，ping/pong 往返，三帧分片消息收发验证
