...
static void test_http(CuTest* tc) {
    ...
    rtnbuf = http_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, NULL != rtnbuf && BIT_CHECK(status, PROT_SLICE_START));
    prots_pkfree(PACK_HTTP, rtnbuf);
    rtnbuf = http_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, NULL != rtnbuf && BIT_CHECK(status, PROT_SLICE));
    prots_pkfree(PACK_HTTP, rtnbuf);
    rtnbuf = http_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, NULL != rtnbuf && BIT_CHECK(status, PROT_SLICE));
    prots_pkfree(PACK_HTTP, rtnbuf);
    rtnbuf = http_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, NULL != rtnbuf && BIT_CHECK(status, PROT_SLICE_END));
    prots_pkfree(PACK_HTTP, rtnbuf);
    prots_udfree(&ud);
...
static void test_redis_unpack(CuTest* tc) {
    ...
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_STRING == pk->prot && 0 == pk->len);
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "+OK\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_STRING == pk->prot && 0 == strcmp("OK", pk->data));
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "-Error message\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_ERROR == pk->prot && 0 == strcmp("Error message", pk->data));
    _redis_pkfree(pk);
    //INTEGER big number
    buffer_appendv(&buf, "%s", "(\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, NULL == pk && BIT_CHECK(status, PROT_ERROR));
    status = 0;
    ...
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_INTEGER == pk->prot && -123 == pk->ival);
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "(12345678\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BIGNUM == pk->prot && 12345678 == pk->ival);
    _redis_pkfree(pk);
    //NULL
    ...
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_NIL == pk->prot);
    _redis_pkfree(pk);
    //BOOL
    buffer_appendv(&buf, "%s", "#\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    ...
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BOOL == pk->prot && 1 == pk->ival);
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "#f\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BOOL == pk->prot && 0 == pk->ival);
    _redis_pkfree(pk);
    //DOUBLE
    buffer_appendv(&buf, "%s", ",\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    ...
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_DOUBLE == pk->prot);
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", ",-inf\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_DOUBLE == pk->prot);
    ...
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, pk == NULL && BIT_CHECK(status, PROT_ERROR));
    status = 0;
    buffer_drain(&buf, buffer_size(&buf));
    ...
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BSTRING == pk->prot && -1 == pk->len);
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "$0\r\n\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BSTRING == pk->prot && 0 == pk->len);
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "$5\r\nhello\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BSTRING == pk->prot && 0 == strcmp("hello", pk->data));
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "!5\r\nError\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BERROR == pk->prot && 0 == strcmp("Error", pk->data));
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "!0\r\n\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_BERROR == pk->prot && 0 == pk->len);
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "!5\r\nError\r");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, NULL == pk && !BIT_CHECK(status, PROT_ERROR));
    ...
    buffer_appendv(&buf, "%s", "=9\r\ntxt:hello\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_VERB == pk->prot && 0 == strcmp("txt", pk->venc) && 0 == strcmp("hello", pk->data));
    _redis_pkfree(pk);
    buffer_appendv(&buf, "%s", "=4\r\ntxt:\r\n");
    pk = redis_unpack(&buf, &ud, &status);
    CuAssertTrue(tc, 0 == buffer_size(&buf) && RESP_VERB == pk->prot && 0 == strcmp("txt", pk->venc) && 0 == pk->len);
    _redis_pkfree(pk);
