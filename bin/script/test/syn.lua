...
local function _timeout()
    local fd, skid = srey.connect(pktype, SSL_NAME.NONE, "127.0.0.1", 15000)
    assert(INVALID_SOCK ~= fd)
    local sdata, ssize = custz.pack(pktype, "this is syn send test.")
    local rdata, rsize = srey.syn_send(fd, skid, sdata, ssize, 0)
    assert(rdata)
    assert(srey.hex("this is syn send test.") == srey.hex(rdata, rsize))
    srey.close(fd, skid)

    fd, skid = srey.connect(pktype, SSL_NAME.NONE, "127.0.0.1", 15001, NET_EV.AUTHSSL)
    assert(INVALID_SOCK ~= fd)
    local auth = srey.syn_ssl_exchange(fd, skid, 1, SSL_NAME.CLIENT)
    assert(auth)
    sdata, ssize = custz.pack(pktype, "this is ssl syn send test.")
    rdata, rsize = srey.syn_send(fd, skid, sdata, ssize, 0)
    assert(rdata)
    assert(srey.hex("this is ssl syn send test.") == srey.hex(rdata, rsize))
    srey.close(fd, skid)

    fd, skid = srey.connect(pktype, SSL_NAME.CLIENT, "127.0.0.1", 15001, NET_EV.AUTHSSL)
    assert(INVALID_SOCK ~= fd)
    sdata, ssize = custz.pack(pktype, "this is ssl syn send test.")
    rdata, rsize = srey.syn_send(fd, skid, sdata, ssize, 0)
    assert(rdata)
