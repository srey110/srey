...
#if WITH_SSL
    char    tls_cbind[32];      // tls-server-end-point：服务端证书 SHA-256 摘要（32字节）
    int32_t tls_cbind_len;      // 0=无TLS或未获取到证书，32=可用
#endif
}pgsql_ctx;

// COPY IN 响应上下文（CopyInResponse）
typedef struct pgpack_copy_in_ctx {
    pgpack_format format;   // 整体数据格式（0=文本, 1=二进制）
    int16_t ncol;           // 列数
}pgpack_copy_in_ctx;

// COPY OUT 响应上下文（CopyOutResponse + 累积的 CopyData）
typedef struct pgpack_copy_out_ctx {
    pgpack_format format;   // 整体数据格式（0=文本, 1=二进制）
    int16_t ncol;           // 列数
    binary_ctx data;        // 累积的原始 CopyData 内容
}pgpack_copy_out_ctx;
