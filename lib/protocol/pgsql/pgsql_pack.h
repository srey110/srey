...
/// <summary>
/// 打包一条或多条 CopyData 消息（'d'），用于 COPY FROM STDIN 数据传输
/// </summary>
...
/// <summary>
/// 打包 CopyDone 消息（'c'），通知服务端 COPY FROM STDIN 数据发送完毕
/// </summary>
/// <param name="size">输出消息字节数</param>
/// <returns>消息数据指针，调用方负责释放</returns>
void *pgsql_pack_copy_done(size_t *size);
/// <summary>
/// 打包 CopyFail 消息（'f'），通知服务端 COPY FROM STDIN 失败并中止
/// </summary>
/// <param name="msg">错误原因描述字符串</param>
/// <param name="size">输出消息字节数</param>
/// <returns>消息数据指针，调用方负责释放</returns>
...
/// </summary>
/// <param name="buf">输出缓冲区，须至少 16 字节</param>
/// <param name="pid">后端进程 ID（BackendKeyData 中获取）</param>
/// <param name="key">后端取消密钥（BackendKeyData 中获取）</param>
