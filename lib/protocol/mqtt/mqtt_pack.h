...
/// <param name="rap">发布保留(MQTT5.0) 1:表示向此订阅转发应用消息时保持消息被发布时设置的保留(RETAIN)标志。0:表示向此订阅转发应用消息时把保留标志设置为0。当订阅建立之后，发送保留消息时保留标志设置为1。</param>
/// <param name="retain">保留操作(MQTT5.0) 0:订阅建立时发送保留消息 1:订阅建立时，若该订阅当前不存在则发送保留消息 2:订阅建立时不要发送保留消息</param>
...
/// <param name="connprops">CONNECT属性(MQTT5.0):</param>
...
/// <param name="willprops">遗嘱属性(MQTT5.0):</param>
...
/// <param name="props">属性(MQTT5.0):</param>
...
/// <param name="version">mqtt_protversion</param>
/// <param name="packid">报文标识符</param>
/// <param name="reasons">原因码列表</param>
