#include "crypt/md2.h"
#include "utils/utils.h"

static const uint8_t s[256] = {
    41, 46, 67, 201, 162, 216, 124, 1, 61, 54, 84, 161, 236, 240, 6,
    19, 98, 167, 5, 243, 192, 199, 115, 140, 152, 147, 43, 217, 188,
    76, 130, 202, 30, 155, 87, 60, 253, 212, 224, 22, 103, 66, 111, 24,
    138, 23, 229, 18, 190, 78, 196, 214, 218, 158, 222, 73, 160, 251,
    245, 142, 187, 47, 238, 122, 169, 104, 121, 145, 21, 178, 7, 63,
    148, 194, 16, 137, 11, 34, 95, 33, 128, 127, 93, 154, 90, 144, 50,
    39, 53, 62, 204, 231, 191, 247, 151, 3, 255, 25, 48, 179, 72, 165,
    181, 209, 215, 94, 146, 42, 172, 86, 170, 198, 79, 184, 56, 210,
    150, 164, 125, 182, 118, 252, 107, 226, 156, 116, 4, 241, 69, 157,
    112, 89, 100, 113, 135, 32, 134, 91, 207, 101, 230, 45, 168, 2, 27,
    96, 37, 173, 174, 176, 185, 246, 28, 70, 97, 105, 52, 64, 126, 15,
    85, 71, 163, 35, 221, 81, 175, 58, 195, 92, 249, 206, 186, 197,
    234, 38, 44, 83, 13, 110, 133, 40, 132, 9, 211, 223, 205, 244, 65,
    129, 77, 82, 106, 220, 55, 200, 108, 193, 171, 250, 36, 225, 123,
    8, 12, 189, 177, 74, 120, 136, 149, 139, 227, 99, 232, 109, 233,
    203, 213, 254, 59, 0, 29, 57, 242, 239, 183, 14, 102, 88, 208, 228,
    166, 119, 114, 248, 235, 117, 75, 10, 49, 68, 80, 180, 143, 237,
    31, 26, 219, 153, 141, 51, 159, 17, 131, 20
};
// MD2 核心变换：更新 state 和 checksum
static void _md2_transform(md2_ctx *md2, const uint8_t *data) {
    int32_t j, k, t;
    for (j = 0; j < 16; ++j) {
        md2->state[j + 16] = data[j];
        md2->state[j + 32] = (md2->state[j + 16] ^ md2->state[j]);
    }
    t = 0;
    for (j = 0; j < 18; ++j) {
        for (k = 0; k < 48; ++k) {
            md2->state[k] ^= s[t];
            t = md2->state[k];
        }
        t = (t + j) & 0xFF;
    }
    t = md2->checksum[15];
    for (j = 0; j < 16; ++j) {
        md2->checksum[j] ^= s[data[j] ^ t];
        t = md2->checksum[j];
    }
}
void md2_init(md2_ctx *md2) {
    ZERO(md2->state, sizeof(md2->state));
    ZERO(md2->checksum, sizeof(md2->checksum));
    md2->lens = 0;
}
void md2_update(md2_ctx *md2, const void *data, size_t lens) {
    const uint8_t *p = (const uint8_t *)data;
    size_t left;
    if (0 == lens) {
        return;
    }
    left = MD2_BLOCK_SIZE - md2->lens;
    if (md2->lens > 0 && lens >= left) {
        memcpy(md2->data + md2->lens, p, left);
        _md2_transform(md2, md2->data);
        md2->lens = 0;
        p += left;
        lens -= left;
    }
    while (lens >= MD2_BLOCK_SIZE) {
        _md2_transform(md2, p);
        p += MD2_BLOCK_SIZE;
        lens -= MD2_BLOCK_SIZE;
    }
    if (lens > 0) {
        memcpy(md2->data + md2->lens, p, lens);
        md2->lens += (uint32_t)lens;
    }
}
void md2_final(md2_ctx *md2, char hash[MD2_BLOCK_SIZE]) {
    int32_t to_pad = MD2_BLOCK_SIZE - md2->lens;
    while (md2->lens < MD2_BLOCK_SIZE) {
        md2->data[md2->lens++] = to_pad;
    }
    _md2_transform(md2, md2->data);
    _md2_transform(md2, md2->checksum);
    memcpy(hash, md2->state, MD2_BLOCK_SIZE);
    secure_zero(md2, sizeof(md2_ctx));
}
