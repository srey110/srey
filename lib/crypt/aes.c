...
static int32_t _aes_key_setup_decrypt(const uint8_t *key, int32_t keybits, uint32_t *rk) {
    uint32_t temp;
    int32_t nrounds, i, j;
    nrounds = _aes_key_setup_encrypt(key, keybits, rk);
...
void aes_init(aes_ctx *aes, const char *key, size_t klens, int32_t keybits, int32_t encrypt) {
    ...
    aes->encrypt = encrypt;
    if (aes->encrypt) {
        aes->nrounds = _aes_key_setup_encrypt(k, keybits, aes->schedule);
    } else {
        aes->nrounds = _aes_key_setup_decrypt(k, keybits, aes->schedule);
    }
    secure_zero(pdkey, sizeof(pdkey));
}
char *aes_crypt(aes_ctx *aes, const void *data) {
    if (aes->encrypt) {
        _aes_encrypt(aes->schedule, aes->nrounds, (const uint8_t *)data, aes->output);
    } else {
        _aes_decrypt(aes->schedule, aes->nrounds, (const uint8_t *)data, aes->output);
    }
