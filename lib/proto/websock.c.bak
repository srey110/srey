...
static inline http_header_ctx *_websock_handshake_clientcheck(void *pack) {
    ...
    if (NULL == pos
        || flens - (pos - fdata) < rsplens
        || 0 != _memicmp(pos, RSP_OK, rsplens)) {
        return NULL;
    }
...
void websock_text(ev_ctx *ev, SOCKET fd, char key[4], const char *data) {
    size_t flens;
    void *frame = _websock_create_pack(1, TEXT, key, (void *)data, strlen(data), &flens);
    ev_send(ev, fd, frame, flens, 0);
}
