#include "strptime.h"
//copy from https://plibc.sourceforge.net/doxygen/strptime_8c-source.html
# define HERE_D_T_FMT "%a %b %e %H:%M:%S %Y"
# define HERE_D_FMT "%y/%m/%d"
# define HERE_T_FMT_AMPM "%I:%M:%S %p"
# define HERE_T_FMT "%H:%M:%S"
#define match_char(ch1, ch2) if (ch1 != ch2) return NULL
#define match_string(cs1, s2) (_memicmp((cs1), (s2), strlen (cs1)) ? 0 : ((s2) += strlen (cs1), 1))
...
#define recursive(new_fmt) \
   (*(new_fmt) != '\0'                                                         \
    && (rp = _strptime_internal (rp, (new_fmt), tm, decided)) != NULL)
...
static char *_strptime_internal(const char *rp, const char *fmt, struct tm *tm,
    ...
    start_over:
        ...
        case 'y':
            ...
            tm->tm_year = (int)(val >= 69 ? val : val + 100);
...
char *_strptime(const char *buf, const char *format, struct tm *tm) {
    enum locale_status decided;
    decided = raw;
    return _strptime_internal(buf, format, tm, &decided);
}
