...
typedef struct skpool_ctx {
    queue_ctx pool;        // 对象池队列（元素 struct sock_ctx *）
    load_trend_ctx trend;  // 负载趋势
}skpool_ctx;
...
static inline void pool_init(skpool_ctx *pool, uint32_t cnt) {
    queue_init(&pool->pool, sizeof(struct sock_ctx *), cnt);
...
static inline void pool_shrink(skpool_ctx *pool, size_t keep) {
    size_t plsize = queue_size(&pool->pool);
    if (load_trend_busy(&pool->trend, plsize, 4, 5)) {
        return;
    }
    if (keep < INIT_EVENTS_CNT) {
        keep = INIT_EVENTS_CNT;
    }
    if (plsize > keep) {
